...
void ofApp::update() {
	...
	for (int i = 0; i < this->noise_seed_list.size(); i++) {
		...
		if (ofGetFrameNum() % 120 < 20) {
			...
			this->noise_param_list[i] += ofMap(ofGetFrameNum() % 120, 0, 20, 0.065, 0.001);
		}
		else if (ofGetFrameNum() % 120 < 100) {

			this->noise_param_list[i] += 0.001;
		}
		else {

			this->noise_param_list[i] += ofMap(ofGetFrameNum() % 120, 100, 120, 0.001, 0.065);
		}
	}
